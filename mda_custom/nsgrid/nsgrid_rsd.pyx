...
ctypedef vector[drealvec] ddrealvec
...
cdef class NSResults(object):
    ...
    cdef void add_neighbors(self, ns_int beadid_i, ns_int beadid_j, dreal dxx, dreal dyy, dreal dzz, dreal distance2) nogil:
        ...
        self.pair_dx_buffer.push_back(dxx)
        self.pair_dx_buffer.push_back(dyy)
        self.pair_dx_buffer.push_back(dzz)
    ...
    def get_pair_dx(self):

        return np.asarray(self.pair_dx_buffer, dtype=np.float64).reshape(self.npairs,3)
    ...
    cdef void create_buffers(self) nogil:
        ...
        for i in range(0, 2*self.npairs, 2):
            ...
            dxx = self.pair_dx_buffer[(i//2)*3]
            dyy = self.pair_dx_buffer[(i//2)*3 + 1]
            dzz = self.pair_dx_buffer[(i//2)*3 + 2]
            ...
            self.dx_buffer[beadid_i].push_back(dxx)
            self.dx_buffer[beadid_i].push_back(dyy)
            self.dx_buffer[beadid_i].push_back(dzz)
...
cdef class _NSGrid(object):
    ...
    cdef ns_int coord2cellid(self, rvec coord) nogil:
        ...
        # RSD: Use EPSILON for situation when coord[XX] is EXACT
        # but cellsize[XX] has floating point error -- e.g., 
        # coord[XX] = 108.0, cellsize[XX] = 21.600000381469727
        # Bin should be 5, but function returns 4 w/o epsilon
        cellx = <ns_int> ((coord[XX]+EPSILON) / self.cellsize[XX])
        celly = <ns_int> ((coord[YY]+EPSILON) / self.cellsize[YY])
        cellz = <ns_int> ((coord[ZZ]+EPSILON) / self.cellsize[ZZ])

        # RSD: This is the part limiting us to periodic systems
        # corrects for when coord[XX] + EPSILON is outside of box
        if cellx >= self.ncells[XX]:
            cellx = 0
        if celly >= self.ncells[YY]:
            celly = 0
        if cellz >= self.ncells[ZZ]:
            cellz = 0

        return cellz * self.cell_offsets[ZZ] + celly * self.cell_offsets[YY] + cellx
    ...
    cdef fill_grid(self, real[:, ::1] coords):
        ...
        with nogil:
            ...
            for i in range(ncoords):
                # THIS WILL ONLY WORK FOR PERIODIC SYSTEMS
...
cdef class FastNS(object):
    ...
    def search(self, search_coords):
        ...
        if not self.periodic:
            raise ValueError("This implementation of nsgrid is only designed for periodic systems. "
                             "self.periodic = {}.".format(self.periodic))
                    ...
                    for yi in range(DIM):
                        for zi in range(DIM):
                            ...
                            for j in range(self.grid.nbeads[cellindex_probe]):
                                ...
                                self.box.fast_dx(&searchcoords_bbox[current_beadid, XX], &self.coords_bbox[bid, XX], dx)
                                d2 = drvec_norm2(dx)
                                if d2 <= cutoff2:
                                    dxx = dx[XX]
                                    dyy = dx[YY]
                        ...
                        for zi in range(DIM):
                            ...
                            for j in range(self.grid.nbeads[cellindex_probe]):
                                ...
                                if bid < current_beadid:
                                    continue
                                # find distance between search coords[i] and coords[bid]
                                self.box.fast_dx(&self.coords_bbox[current_beadid, XX], &self.coords_bbox[bid, XX],dx)
                                d2 = drvec_norm2(dx)
